class QuickAnswerApp {
    ...
    async extractPageContent() {
        ...
        const results = await chrome.scripting.executeScript({
            ...
            func: () => {
                try {
                    ...
                    // Helper logic to clean elements
                    const cloneAndClean = (node) => {
                        const clone = node.cloneNode(true);
                        const selectors = [
                            'script', 'style', 'noscript', 'link',
                            'nav', 'footer', 'aside', '.ads', '.comments', 
                            '.cookie-banner', '.popup', '.overlay', '.modal',
                            '[role="complementary"]', '#onetrust-banner-sdk', '#onetrust-consent-sdk',
                            'iframe' // Don't include nested iframes in the HTML string, we process them separately via executeScript
                        ];
                        clone.querySelectorAll(selectors.join(',')).forEach(el => el.remove());
                        return clone;
                    };

                    const cleanBody = cloneAndClean(document.body);
                    const textLength = cleanBody.innerText.trim().length;

                    // Filter empty frames
                    // Allow smaller frames if they have substantial structure, but skip empty ones
        ...
        const validFrames = results
            ...
            .filter(res => res && res.success && res.html);
        ...
        // Combine content
        const combinedHtml = validFrames.map(f => `
            <div class="frame-content" data-url="${f.url}">
                ${f.html}
            </div>
        `).join('\n<hr/>\n');

        this.lastTitle = validFrames[0].title || tab.title;
